...
\section{Desain Arsitektur Sistem Perangkat Lunak Audit Energi}
Desain arsitektur sistem disusun untuk menggambarkan alur interaksi antarkomponen utama dalam perangkat lunak audit energi, mulai dari pengguna hingga ke pengolahan data. Desain ini penting untuk memastikan bahwa struktur sistem yang diusulkan dapat mendukung kebutuhan fungsional yang telah dirumuskan. Hubungan antarkomponen juga dirancang untuk mencerminkan alur kerja yang terjadi selama proses audit energi. Alur interaksi antarkomponen ini diilustrasikan melalui diagram arsitektur sistem yang ditampilkan pada Gambar~\ref{gambar:arch}.

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
        \includegraphics[width=0.25\textwidth]{image/arch}
    \caption{Diagram Arsitektur Sistem Perangkat Lunak Audit Energi}
    \label{gambar:arch}
\end{figure}

Diagram arsitektur sistem pada Gambar~\ref{gambar:arch} menampilkan komponen utama sistem, yaitu Pengguna, Website Frontend, Flask Backend, dan Firestore Firebase Database. Pengguna berinteraksi dengan sistem melalui Website Frontend, yang menyediakan antarmuka untuk input data audit, pemantauan hasil audit, serta akses ke fitur-fitur lainnya. Website Frontend akan meneruskan permintaan \textit{(request)} ke Flask backend untuk diproses sesuai dengan logika bisnis yang telah dirancang. Flask Backend bertanggung jawab untuk memproses logika aplikasi, melakukan perhitungan indikator kinerja energi, serta mengelola alur kerja audit secara keseluruhan. Data yang diproses oleh Flask Backend disimpan dan dikelola dalam Firestore Firebase Database, yang menyediakan penyimpanan terpusat untuk data audit, histori audit, serta hasil perhitungan. Dengan desain arsitektur ini, sistem audit energi dapat berjalan secara efisien dan mendukung kebutuhan fungsional yang telah dirumuskan pada tahap analisis kebutuhan sebelumnya.
